...
TAutoConsoleVariable<bool> CVarInteractDebugDrawing(TEXT("game.interaction.DebugDraw"), false, 
...
void URogueInteractionComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	bool bEnabledDebugDraw = CVarInteractDebugDrawing.GetValueOnGameThread();
	...
	for (FOverlapResult& Overlap : Overlaps)
	{
		...
		if (bEnabledDebugDraw)
		{
			...
			FString DebugString = FString::Printf(TEXT("Weight: %f, Dot: %f, Distance To: %f"), Weight, NormalizedDotResult, NormalizedDistanceTo);
